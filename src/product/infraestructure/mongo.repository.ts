...
export class ProductMongoRepository implements ProductRepository {
  async save(uuid: string, product: Product): Promise<void> {
    await ProductModel.findOneAndUpdate({ uuid }, this.toPersistence(product), {
      upsert: true,
      new: true,
    });
  }
  ...
  async findAllWithCategory(): Promise<ProductWithCategory[]> {
    ...
    return products.map(({ category, ...product }) => {
      const productEntity = this.toDomain(product as ProductDoc);

      return Object.assign(productEntity, {
        category: {
          name: category.name,
        },
